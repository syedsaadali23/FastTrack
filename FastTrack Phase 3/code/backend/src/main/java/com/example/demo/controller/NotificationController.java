...
public class NotificationController {
        ...
        if ("ADMIN".equals(user.getRole())) {
            ...
            for (User u : userRepository.findAll()) {
                if (u.getEmail() != null && !u.getEmail().isEmpty()) {
                    emailService.sendEmail(u.getEmail(), title, body);
                }
            }
    ...
    public ResponseEntity<?> approveNotification(@PathVariable Long id, @RequestBody Map<String, String> data) {
        ...
        if ("PENDING".equals(n.getStatus())) {
            ...
            for (User u : userRepository.findAll()) {
                if (u.getEmail() != null && !u.getEmail().isEmpty()) {
                    emailService.sendEmail(u.getEmail(), n.getTitle(), n.getBody());
                }
            }
        }
        csvExportService.exportAll();
    ...
    public ResponseEntity<?> rejectNotification(@PathVariable Long id, @RequestBody Map<String, String> data) {
        ...
        if ("PENDING".equals(n.getStatus())) {
            ...
            Optional<User> senderOpt = userRepository.findByUsername(n.getSenderUsername());
            if (senderOpt.isPresent() && senderOpt.get().getEmail() != null) {
                emailService.sendEmail(senderOpt.get().getEmail(), rejection.getTitle(), rejection.getBody());
            }
        }
        csvExportService.exportAll();
