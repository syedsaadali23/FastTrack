                ...
                {results.teams.map((t) => (
                  <Link key={t.id} to={`/teams/${t.id}`} className="rounded-lg border border-border p-3 hover:bg-muted/30">
                    <div className="flex items-start justify-between">
                      ...
                      <span className="text-[10px] font-black bg-primary/10 text-primary px-1.5 py-0.5 rounded border border-primary/20">MMR: {t.skillLevel}</span>
                    </div>
                    <p className="text-xs text-muted-foreground">{t.sport}{t.teamCap ? ` · ${t.members.length}/${t.teamCap}` : ""}</p>
